...
namespace Unity.UIWidgets.ui {
    public class uiMatrix3 : IEquatable<uiMatrix3> {
        ...
        public static uiMatrix3 makeScale(float scale) {
            var m = new uiMatrix3();
        ...
        public static uiMatrix3 makeAll(
        ...
        public bool isSimilarity(float tol = uiScalarUtils.kScalarNearlyZero) {
            ...
            if ((mask & TypeMask.kAffine_Mask) == 0) {
                return !uiScalarUtils.ScalarNearlyZero(mx) &&
                       uiScalarUtils.ScalarNearlyEqual(Mathf.Abs(mx), Mathf.Abs(my));
            }

            float sx = this.fMat[kMSkewX];
            float sy = this.fMat[kMSkewY];

            if (uiScalarUtils.is_degenerate_2x2(mx, sx, sy, my)) {
                return false;
            }

            // upper 2x2 is rotation/reflection + uniform scale if basis vectors
            // are 90 degree rotations of each other
            return (uiScalarUtils.ScalarNearlyEqual(mx, my, tol) && uiScalarUtils.ScalarNearlyEqual(sx, -sy, tol))
                   || (uiScalarUtils.ScalarNearlyEqual(mx, -my, tol) && uiScalarUtils.ScalarNearlyEqual(sx, sy, tol));
        }

        public bool preservesRightAngles(float tol = uiScalarUtils.kScalarNearlyZero) {
            TypeMask mask = this.getType();
            ...
            var dot = mx * sx + sy * my;
            return uiScalarUtils.ScalarNearlyZero(dot, tol * tol);
        }
        ...
        public void setSinCos(float sinV, float cosV, float px, float py) {
            ...
            this.fMat[kMTransX] = uiScalarUtils.sdot(sinV, py, oneMinusCosV, px);

            this.fMat[kMSkewY] = sinV;
            this.fMat[kMScaleY] = cosV;
            this.fMat[kMTransY] = uiScalarUtils.sdot(-sinV, px, oneMinusCosV, py);
        ...
        public void setConcat(uiMatrix3 a, uiMatrix3 b) {
        ...
        public bool invert(uiMatrix3 inverse) {
            if (this.isIdentity()) {
                if (inverse != null) {
                    inverse.reset();
                }
        ...
        public static bool operator ==(uiMatrix3 a, uiMatrix3 b) {
        ...
        public static uiMatrix3 concat(uiMatrix3 a, uiMatrix3 b) {
            uiMatrix3 result = new uiMatrix3();
        ...
        public bool isFinite() {
            return uiScalarUtils.ScalarsAreFinite(this.fMat, 9);
        }
        ...
        int computeTypeMask() {
            ...
            int m00 = uiScalarUtils.ScalarAs2sCompliment(this.fMat[kMScaleX]);
            int m01 = uiScalarUtils.ScalarAs2sCompliment(this.fMat[kMSkewX]);
            int m10 = uiScalarUtils.ScalarAs2sCompliment(this.fMat[kMSkewY]);
            int m11 = uiScalarUtils.ScalarAs2sCompliment(this.fMat[kMScaleY]);
        ...
        static void Rot_xy(uiMatrix3 m, float sx, float sy, out float resX, out float resY) {
            ...
            resX = uiScalarUtils.sdot(sx, m.fMat[kMScaleX], sy, m.fMat[kMSkewX]);
            resY = uiScalarUtils.sdot(sx, m.fMat[kMSkewY], sy, m.fMat[kMScaleY]);
        }

        static void RotTrans_xy(uiMatrix3 m, float sx, float sy, out float resX, out float resY) {
            D.assert((m.getType() & (TypeMask.kAffine_Mask | TypeMask.kPerspective_Mask)) == TypeMask.kAffine_Mask);

            resX = uiScalarUtils.sdot(sx, m.fMat[kMScaleX], sy, m.fMat[kMSkewX]) + m.fMat[kMTransX];
            resY = uiScalarUtils.sdot(sx, m.fMat[kMSkewY], sy, m.fMat[kMScaleY]) + m.fMat[kMTransY];
        }

        static void Persp_xy(uiMatrix3 m, float sx, float sy, out float resX, out float resY) {
            D.assert(m.hasPerspective());

            float x = uiScalarUtils.sdot(sx, m.fMat[kMScaleX], sy, m.fMat[kMSkewX]) +
                      m.fMat[kMTransX];
            float y = uiScalarUtils.sdot(sx, m.fMat[kMSkewY], sy, m.fMat[kMScaleY]) +
                      m.fMat[kMTransY];
            float z = uiScalarUtils.sdot(sx, m.fMat[kMPersp0], sy, m.fMat[kMPersp1]) +
                      m.fMat[kMPersp2];
            if (z != 0) {
                z = 1 / z;
            }
        ...
        static void Persp_pts(uiMatrix3 m, Offset[] dst, Offset[] src, int count) {
            ...
            if (count > 0) {
                for (int i = 0; i < count; ++i) {
                    var sy = src[i].dy;
                    var sx = src[i].dx;
                    var x = uiScalarUtils.sdot(sx, m.fMat[kMScaleX], sy, m.fMat[kMSkewX]) +
                            m.fMat[kMTransX];
                    var y = uiScalarUtils.sdot(sx, m.fMat[kMSkewY], sy, m.fMat[kMScaleY]) +
                            m.fMat[kMTransY];
                    var z = uiScalarUtils.sdot(sx, m.fMat[kMPersp0], sy, m.fMat[kMPersp1]) +
                            m.fMat[kMPersp2];
                    if (z != 0) {
                        z = 1 / z;
                    }
        ...
        bool invertNonIdentity(uiMatrix3 inv) {
            ...
            float invDet = uiScalarUtils.inv_determinant(this.fMat, isPersp);
            ...
            if (applyingInPlace || null == tmp) {
                tmp = new uiMatrix3(); // we either need to avoid trampling memory or have no memory
            }
        ...
        public bool Equals(uiMatrix3 other) {
        ...
        public override bool Equals(object obj) {
            ...
            return this.Equals((uiMatrix3) obj);
        }
        ...
        public override string ToString() {
            return "uiMatrix3(" + string.Join(",", Array.ConvertAll(this.fMat, i => i.ToString())) + ")";
        }
    ...
    class uiScalarUtils {
        ...
        public static float inv_determinant(float[] mat, int isPerspective) {
            ...
            if (isPerspective != 0) {
                det = mat[uiMatrix3.kMScaleX] *
                      dcross(mat[uiMatrix3.kMScaleY], mat[uiMatrix3.kMPersp2],
                          mat[uiMatrix3.kMTransY], mat[uiMatrix3.kMPersp1])
                      +
                      mat[uiMatrix3.kMSkewX] *
                      dcross(mat[uiMatrix3.kMTransY], mat[uiMatrix3.kMPersp0],
                          mat[uiMatrix3.kMSkewY], mat[uiMatrix3.kMPersp2])
                      +
                      mat[uiMatrix3.kMTransX] *
                      dcross(mat[uiMatrix3.kMSkewY], mat[uiMatrix3.kMPersp1],
                          mat[uiMatrix3.kMScaleY], mat[uiMatrix3.kMPersp0]);
            }
            else {
                det = dcross(mat[uiMatrix3.kMScaleX], mat[uiMatrix3.kMScaleY],
                    mat[uiMatrix3.kMSkewX], mat[uiMatrix3.kMSkewY]);
            }
