...
namespace Unity.UIWidgets.ui {
    public class uiPath : PoolItem {
        ...
        public override string ToString() {
            var sb = new StringBuilder("uiPath: count = " + this._commands.Count);
            ...
            while (i < this._commands.Count) {
                ...
                switch (cmd) {
                    ...
                    case uiPathCommand.winding:
                        sb.Append(", winding(" + (uiPathWinding) this._commands[i + 1] + ")");
                        i += 2;
                        break;
                    default:
        ...
        public void winding(uiPathWinding dir) {
        ...
        public void arcTo(float x1, float y1, float x2, float y2, float radius) {
            ...
            float dx1 = x2 - x1;
            float dy1 = y2 - y1;
            uiPathUtils.normalize(ref dx0, ref dy0);
            uiPathUtils.normalize(ref dx1, ref dy1);
            ...
            if (cross > 0.0f) {
                ...
                dir = uiPathWinding.clockwise;
            } else {
                cx = x1 + dx0 * d + -dy0 * radius;
                cy = y1 + dy0 * d + dx0 * radius;
                a0 = Mathf.Atan2(-dx0, dy0);
                a1 = Mathf.Atan2(dx1, -dy1);
                dir = uiPathWinding.counterClockwise;
            }
        ...
        public void arcTo(Rect rect, float startAngle, float sweepAngle, bool forceMoveTo = true) {
            ...
            this._addArcCommands(0, 0, 1, startAngle, startAngle + sweepAngle,
                sweepAngle >= 0 ? uiPathWinding.clockwise : uiPathWinding.counterClockwise, forceMoveTo, mat);
        }
        ...
        void _addArcCommands(
            ...
            uiPathWinding dir, bool forceMoveTo, Matrix3 transform = null) {
            // Clamp angles
            float da = a1 - a0;
            if (dir == uiPathWinding.clockwise) {
            ...
            if (dir == uiPathWinding.counterClockwise) {
                kappa = -kappa;
            }

            uiPathCommand move = (forceMoveTo || this._commands.Count == 0) ? uiPathCommand.moveTo : uiPathCommand.lineTo;
        ...
        public void addArc(float cx, float cy, float r, float a0, float a1, uiPathWinding dir, bool forceMoveTo = true) {
        ...
        public void addPath(uiPath path, Matrix3 transform = null) {
            ...
            while (i < path._commands.Count) {
                var cmd = (uiPathCommand) path._commands[i];
                switch (cmd) {
                    case uiPathCommand.moveTo: {
        ...
        public bool contains(Offset point) {
            ...
            float commandx = 0;
            float commandy = 0;
            uiPathWinding winding = uiPathWinding.counterClockwise;
            ...
            while (i < this._commands.Count) {
                ...
                switch (cmd) {
                    case uiPathCommand.moveTo:
                        ...
                        if (w != 0) {
                            totalW += winding == uiPathWinding.counterClockwise ? w : -w;
                            w = 0;
                        }
                    ...
                    case uiPathCommand.close:
                        ...
                        break;
                    case uiPathCommand.winding:
                        winding = (uiPathWinding) this._commands[i + 1];
                        i += 2;
                        break;
                    default:
                        D.assert(false, () => "unknown cmd: " + cmd);
    ...
    class _uiConic {
        ...
        void _chop(out _uiConic c1, out _uiConic c2) {
    ...
    static class uiPathUtils {
        ...
        public static float polyArea(List<uiPathPoint> points, int s, int npts) {
            float area = 0;
