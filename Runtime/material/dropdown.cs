...
namespace Unity.UIWidgets.material {
    ...
    class _DropdownMenuPainter : AbstractCustomPainter {
      ...
      public override void paint(Canvas canvas, Size size) {
        float selectedItemOffset = this.selectedIndex * DropdownConstants._kMenuItemHeight + Constants.kMaterialListPadding.top;
        ...
        FloatTween bottom = new FloatTween(
          begin: (top.begin + DropdownConstants._kMenuItemHeight).clamp(DropdownConstants._kMenuItemHeight, size.height),
          end: size.height
        );
    ...
    class _DropdownScrollBehavior : ScrollBehavior {
      ...
      public override Widget buildViewportChrome(BuildContext context, Widget child, AxisDirection axisDirection) => child;
    
      public override ScrollPhysics getScrollPhysics(BuildContext context) => new ClampingScrollPhysics();
    }
    ...
    class _DropdownMenu<T> : StatefulWidget where T: Diagnosticable {
      ...
      public override State createState() => new _DropdownMenuState<T>();
    }
    ...
    class _DropdownRoute<T> : PopupRoute where T: Diagnosticable {
      ...
      public override TimeSpan transitionDuration {
          get {
              return DropdownConstants._kDropdownMenuDuration;
          }
      }
      ...
      public override Widget buildPage(BuildContext context, Animation<float> animation, Animation<float> secondaryAnimation) {
        ...
        float? menuTop = (buttonTop - selectedItemOffset) - (DropdownConstants._kMenuItemHeight - this.buttonRect.height) / 2.0f;
        float preferredMenuHeight = (this.items.Count * DropdownConstants._kMenuItemHeight) + Constants.kMaterialListPadding.vertical;
        ...
        if (this.scrollController == null) {
          float scrollOffset = preferredMenuHeight > maxMenuHeight ? Mathf.Max(0.0f, selectedItemOffset - (buttonTop - menuTop) ?? 0.0f) : 0.0f;
    ...
    public class DropdownButton<T> : StatefulWidget where T: Diagnosticable {
        public DropdownButton(
            ...
            D.assert(items == null || value == null ||
                     items.Where<DropdownMenuItem<T>>((DropdownMenuItem<T> item) => item.value.Equals(value)).ToList().Count == 1);
        }
      ...
      public override State createState() => new _DropdownButtonState<T>();
    }
    ...
    class _DropdownButtonState<T> : State<DropdownButton<T>>, WidgetsBindingObserver where T: Diagnosticable {
      ...
      void _updateSelectedIndex() {
        ...
        D.assert(this.widget.value == null ||
          this.widget.items.Where((DropdownMenuItem<T> item) => item.value.Equals(this.widget.value)).ToList().Count == 1);
      ...
      TextStyle _textStyle {
          get {
              return this.widget.style ?? Theme.of(this.context).textTheme.subhead;
          }
      }
